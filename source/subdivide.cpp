...
void subdivide_edgeDiff(MatrixXi &F, MatrixXd &V, MatrixXd &N, MatrixXd &Q, MatrixXd &O, MatrixXd* S,
                        ...
                        std::vector<Vector3i> &face_edgeOrients, std::vector<Vector3i> &face_edgeIds, std::vector<int> &sharp_edges,
                        std::map<int, int> &singularities, std::vector<int> &Boundary_edges) {
    struct EdgeLink {
        int id;
        double length;
        Vector2i diff;
    ...
    for (int i = 0; i < E2E.size(); ++i) {
        ...
        if (abs(diff[0]) > comp || abs(diff[1]) > comp) {
            int other = E2E[i];
            if (other == -1 || other > i) {
                EdgeLink e;
                e.id = i;
    ...
    */
    ...
    while (!queue.empty()) {
        ...
        int Boundary_eid = Boundary_edges[e0];
        int Boundary_eid01 = Boundary_edges[f0 * 3 + (e0 + 1) % 3];
        int Boundary_eid02 = Boundary_edges[f0 * 3 + (e0 + 2) % 3];
        int Boundary_eid0 = Boundary_eid;
        int Boundary_eid1 = Boundary_eid;
        ...
        Boundary_edges[f0 * 3] = sharp_eid0p;
        Boundary_edges[f0 * 3 + 1] = Boundary_eid02;
        Boundary_edges[f0 * 3 + 2] = Boundary_eid0;
        ...
        if (!is_boundary) {
            ...
            int Boundary_eid11 = Boundary_edges[f1 * 3 + (e1 + 1) % 3];
            int Boundary_eid12 = Boundary_edges[f1 * 3 + (e1 + 2) % 3];
            ...
            Boundary_edges[f1 * 3] = Boundary_eid0;
            Boundary_edges[f1 * 3 + 1] = Boundary_eid11;
            Boundary_edges[f1 * 3 + 2] = sharp_eid1p;
            ...
            Boundary_edges[f2 * 3] = sharp_eid1p;
            Boundary_edges[f2 * 3 + 1] = Boundary_eid12;
            Boundary_edges[f2 * 3 + 2] = Boundary_eid1;
        ...
        Boundary_edges[f3 * 3] = Boundary_eid1;
        Boundary_edges[f3 * 3 + 1] = Boundary_eid01;
        Boundary_edges[f3 * 3 + 2] = sharp_eid0p;
...
#undef sE2E
        ...
        auto schedule = [&](int f) {
            for (int i = 0; i < 3; ++i) {
                if (abs(diffs[f * 3 + i][0]) > comp || abs(diffs[f * 3 + i][1]) > comp) {
                    EdgeLink e;
                    e.id = f * 3 + i;
