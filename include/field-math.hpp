...
inline std::pair<Vector3d, Vector3d> compat_orientation_extrinsic_41(const Vector3d &q0,
                                                                    const Vector3d &n0,
                                                                    const Vector3d &q1,
                                                                    const Vector3d &n1) {
    const Vector3d A[4] = {q0, n0.cross(q0),-q0,-n0.cross(q0)};
    const Vector3d B[2] = {q1, n1.cross(q1)};
    ...
    for (int i = 0; i < 4; ++i) {
        double score = std::abs(A[i].dot(B[0]));
        if (score > best_score + 1e-6) {
            best_a = i;
            best_b = 0;
            best_score = score;
